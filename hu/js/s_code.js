/* Omniture JavaScript File for external HTML tracking

   Please remember to move from dev report suite when live!

   Configuration Features:
   - cid URL parameter captured as s.campaign
   - intcmp URL parameter captured as s.eVar
   - hour and day parting captured in s.prop1/s.eVar1 and s.prop2/s.eVar2
   - pageName generated from URL using plugin
   
   Users must set the siteID, channel and pageNamePrefix.
*/

//TODO: Enter the live report suite here
 var s_siteID = 'cnuk'; //When Live
//var s_siteID = 'dev'; // When in development


//The channel - this will be marketing (for internal marketing) or advertisers (for external advertisers)
//Note: Setting this will populate prop15 (marketing) and prop17 (advertisers) with the pagenameprefix below
var channel = 'marketing';

//Specify a useful page name prefix to appear on all pages
var pageNamePrefix = 'CN-Buddy-Network-2014';

var s_account = 'turneruk' + s_siteID;
if (s_siteID != 'dev') {
  s_account = s_account + ',turnerukglobal';
}


/*
    Lynchpin: Function to track page view with a given page name
*/
function lypn_trackPageView(pageName) {
    if (!pageName) {
        return;
    }
    
    var page = new Object();
    page.pageName = s.pageName + ":" + pageName;
    s.t(page);
}


/* Dynamic siteID and s_account selection code */
var s_server = window.location.host.toLowerCase().replace('www.','');
var s_cookieDomainPeriods = s_server.split('.').length;
...
if (channel == 'marketing') {
    s.prop15 = pageNamePrefix;
}
else if (channel == 'advertisers') {
    s.prop17 = pageNamePrefix;
}
...
s.cookieDomainPeriods=s_cookieDomainPeriods;
s.currencyCode="GBP";
s.trackDownloadLinks=true;
s.trackExternalLinks=true;
s.trackInlineStats=true;
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,jpeg,jpg";
s.linkInternalFilters="javascript:,"+s_server;
s.linkLeaveQueryString=true;
//Homepage in-page interaction
...
s.visitorNamespace="turneruk";
s.dc="112";
s.trackingServer="tenilstats.turner.com"
/*s.trackingServerSecure="NEED AN ENTRY WHEN SSL IS BEING USED"*/
...
function s_doPlugins(s) {
  s.eVar1=s.prop1=s.getTimeParting('h','0',new Date().getFullYear());
  s.eVar2=s.prop2=s.getTimeParting('d','0',new Date().getFullYear());
  if(!s.campaign) {
    ...
    //Get previous page
    if (previousPage == '') {
        previousPage = s.getPreviousValue(s.pageName,'gpv_pn');
    }
    
  //Try ? then try #
    var intcmp;
    if(!intcmp) {
      intcmp=s.getQueryParam('intcmp');
        if (!intcmp) {
            intcmp=s.epa(getHashQueryParam('intcmp'));
        }
  }
    
    if (intcmp) {
        //If the internal campaign is the carousel and the previous page was the home page,
        //send to the home page eVars rather than eVar17
        var homePageRegex = '^' + s_siteID + ':home$';
        if (intcmp == 'Carousel' && previousPage.match(homePageRegex)) {
            s.eVar21 = intcmp;
            s.eVar22 = intcmp;
        }
    ...
    //Get homepage tracking
    //Decode URI as IE sometimes includes %20s etc.
    var homepageParameter = decodeURI(getHashQueryParam('homepage'));
    if (homepageParameter) {
        var homepageCodeRegex = /(.*?) -/;
        
        var homepageMatches = homepageCodeRegex.exec(homepageParameter);
        if (homepageMatches && homepageMatches.length > 1) {
            //Record the overview
            s.eVar21 = homepageMatches[1];
            //Record all the detail
            s.eVar22 = homepageParameter;
        }
        else {
            //We couldn't find anything which matches our "more detail" regex (i.e. a space then hyphen)
            s.eVar21 = homepageParameter;
            s.eVar22 = homepageParameter;
        }
    }
...
+"queryVarsList,d=s.siteID+':'+pageNamePrefix,n=d?d:'',q=z<0?'':v.substring(z+1),p=v.sub"
